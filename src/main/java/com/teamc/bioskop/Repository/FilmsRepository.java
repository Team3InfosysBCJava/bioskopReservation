...
public interface FilmsRepository extends JpaRepository<Films, Long> {
    ...
    @Query("SELECT f FROM Films f WHERE CONCAT(f.filmId, ' ', f.isPlaying, ' ', f.name, ' ') LIKE %?1% ORDER BY f.name ASC")
    public Page<Films> Search(String keyword, Pageable pageable);
